...
folder = 'Results_AEFMEF_EV_co2_'+date
...
for ev_name in ['UniversalHome', 'HighHome', 'LowHome_HighWork']:
    ...
    for year in year_set:
        save_str = folder+'/uncontrolled_15min_co2_EVs_'+ev_name+'_year'+str(year)
        print(save_str)
        signal_here = None
...
for reg in ['nonreg']:
    for control in ['AEF', 'MEF']:
        for ev_name in ['UniversalHome', 'HighHome', 'LowHome_HighWork']:
            dpdfs_all = {}
            for year in year_set:
                save_str = folder+'/controlled_15min_co2_EVs_'+control+'_weekdayweekend_noevbase'+'_'+ev_name+'_'+reg+'_year'+str(year)
                print(save_str)
                signalyear = year_signal_mapping[year]
                signal_here = None
                dpdfs_all[year] = run_dispatch_wrapped(save_str, gd, year, signal_here, time_array, date, ev_uncontrolled=False, ev_name=ev_name, ev_control_name_weekday=control+'_weekday_'+str(signalyear), ev_control_name_weekend=control+'_weekend_'+str(signalyear), ev_reg=reg, ev_norm=True, ev_control_date=ev_control_date, co2_dol_per_kg=co2_dol_per_kg)
