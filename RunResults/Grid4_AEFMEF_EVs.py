...
folder = 'Results_AEFMEF_EV_'+date
...
def run_dispatch_wrapped(save_str, gd, year, signal_here, time_array, date, hourly_signal_weekday=None, hourly_signal_weekend=None, ev_uncontrolled=False, ev_name='HighHome', ev_control_name_weekday='AEF_weekday_2020', ev_control_name_weekend='AEF_weekend_2020', ev_reg='nonreg', ev_norm=True, ev_control_date='20230731', baseyear=2019):
    ...
    grid1.run_dispatch(save_str, verbose=False, time_array=time_array, result_date=date, coal_downtime=False, year=year, co2_dol_per_kg=0)
...
for ev_name in ['UniversalHome', 'HighHome', 'LowHome_HighWork']:
    ...
    for year in year_set:
        save_str = folder+'/uncontrolled_15min_EVs_'+ev_name+'_year'+str(year)
        print(save_str)
        signal_here = None
        dpdfs_all[year] = run_dispatch_wrapped(save_str, gd, year, signal_here, time_array, date, ev_uncontrolled=True, ev_name=ev_name, ev_norm=True, ev_control_date=ev_control_date)
...
for reg in ['nonreg']:
    for control in ['AEF', 'MEF']:
        for ev_name in ['UniversalHome', 'HighHome', 'LowHome_HighWork']:
            dpdfs_all = {}
            for year in year_set:
                save_str = folder+'/controlled_15min_EVs_'+control+'_weekdayweekend_noevbase'+'_'+ev_name+'_'+reg+'_year'+str(year)
                print(save_str)
                signalyear = year_signal_mapping[year]
                signal_here = None
                dpdfs_all[year] = run_dispatch_wrapped(save_str, gd, year, signal_here, time_array, date, ev_uncontrolled=False, ev_name=ev_name, ev_control_name_weekday=control+'_weekday_'+str(signalyear), ev_control_name_weekend=control+'_weekend_'+str(signalyear), ev_reg=reg, ev_norm=True, ev_control_date=ev_control_date)
