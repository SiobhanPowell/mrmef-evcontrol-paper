...
for reg in ['nonreg']:
    for deltaMW in [5000, 10000]:
        for ev_name in ['UniversalHome', 'HighHome', 'LowHome_HighWork']:
            dpdfs_all = {}
            for year in year_set:
                save_str = folder+'/controlled_15min_EVs_MRMEF_delta'+str(deltaMW)+'_5yearsahead_weekdayweekend_noevbase'+'_'+ev_name+'_'+reg+'_year'+str(year)
                print(save_str)
                ...
                signal_here = None
                dpdfs_all[year] = run_dispatch_wrapped(save_str, gd, year, signal_here, time_array, date, ev_uncontrolled=False, ev_name=ev_name, ev_control_name_weekday=ev_control_name_weekday, ev_control_name_weekend=ev_control_name_weekend, ev_reg=reg, ev_norm=True, ev_control_date=ev_control_date)
