...
signals_df_lrmef = pd.read_csv('Data/signals_df_mrmef_storagebefore_noevbase_co2price_1dolperkg_20240208.csv', index_col=0)

date = '20240208'
folder = 'Outputs_EVControl_CO2_mrmef_'+date+'/'
if not os.path.isdir(folder):
    os.mkdir(folder)

cols_to_optimize = ['MRMEF_delta'+str(deltaMW)+'_startyear'+str(year)+'_5yearsahead_'+weekday for deltaMW in [5000, 10000] for year in [2023, 2028, 2033] for weekday in ['weekday', 'weekend']]
...
for i in range(num_runs):
    ...
    for name, df in {'home':df_home, 'work':df_work}.items():
        ...
        for col in cols1:
            obj = np.copy(np.repeat(signals_df_lrmef[col].values, time_steps_per_hour))
