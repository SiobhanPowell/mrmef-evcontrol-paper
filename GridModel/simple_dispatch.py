"""Extension of model originally published by Thomas Deetjen : https://github.com/tdeetjen/simple_dispatch
The second class, StorageModel, was written by Siobhan Powell.  The first class, GridModel, was developed with Sonia Martin.
...
class StorageModel(object):
    """Added by Siobhan Powell. Storage can be used in two ways: 1) to flatten net demand,
    and 2) to catch overflow in the dispatch. For the second application, here we calculate the minimum size needed.

    :param df: Dataframe which contains in problem (1) the net demand at each hour of the year,
    in problem (2) the unmet demand after dispatch to be covered by the storage
    :type df: Pandas dataframe

    :param res: Result of optimization
    :type res: cvxpy object

    :param min_capacity: Capacity (energy) of minimum sufficient storage for problem (2)
    :type min_capacity: float
    :param min_maxrate: Rate (mw) of minimum sufficient storage for problem (2)
    :type min_maxrate: float
    :param max_rate: Required rate (mw) for problem (1)
    :type max_rate: float
    """
    def __init__(self, storage_df):
        ...
        self.df = storage_df
        self.res = None
        self.min_capacity = numpy.inf
        
        self.min_maxrate = None
        self.maxrate = None
        self.max_rate = None

    def calculate_minbatt_forcapacity(self):
        ...
        self.max_rate = self.df['demand'].max()
        if numpy.abs(self.max_rate)>0:

            cap = cp.Variable(1)  # storage capacity
            en = cp.Variable(len(self.df))  # energy stored at each time
            r = cp.Variable(len(self.df)-1)  # charge or discharge rate at each time
            max_rate = cp.Variable(1)  # rate
            comb_demand = cp.Variable(len(self.df))  # net demand

            constraints = [en[0] == cap, r >= -max_rate, r <= max_rate, en >= 0, en <= cap,
                           cap == 4*max_rate, max_rate >= self.max_rate]  # Force capacity = 4*rate for 4 hour storage
            for i in range(len(self.df)-1):
                constraints.append(en[i+1] == en[i] + r[i])  # charging or discharging changes energy
                constraints.append(self.df['unclipped_demand'].values[i] + r[i] <= self.df['capacity_clipval'].values[i])
                if self.df['demand'].values[i] > 0:
                    constraints.append(r[i] == -self.df['demand'].values[i])  # must cover the unmet demand
                    

            prob = cp.Problem(cp.Minimize(cap), constraints)  # smallest battery that does this, with some normalization
#             prob = cp.Problem(cp.Minimize(cap+0.0001*cp.norm1(r)), constraints)  # smallest battery that does this, with some normalization
            print('Solving optimization.')
            self.res = prob.solve(solver='MOSEK')#, verbose=True)

#             print('Solving optimization. Note: using ECOS for now since MOSEK install not working.')
#             self.res = prob.solve(solver='ECOS')
            self.min_capacity = cap.value
            ...
            self.min_capacity = 0
            self.min_maxrate = 0
            self.df['battery_operation'] = 0
            
    def calculate_operation_beforecapacity(self, cap, max_rate, allow_negative=False):
        """Calculate the minimum battery capacity, operating (as a simplification) as if there were one large battery
        satisfying this load, so the battery can only charge or discharge at once."""
        en = cp.Variable(len(self.df))  # energy
        r = cp.Variable(len(self.df)-1)  # rate
        comb_demand = cp.Variable(len(self.df))  # net demand

        constraints = [en[0] == cap, r >= -max_rate, r <= max_rate, en >= 0, en <= cap]  # capacity and max rate are not variables
        for i in range(len(self.df)-1):
            constraints.append(en[i+1] == en[i] + r[i])  # charging or discharging changes energy
        ...
        print('Solving optimization.')
        self.res = prob.solve(solver='MOSEK')
#         print('Solving optimization. Note: using ECOS for now since MOSEK install not working.')
#         self.res = prob.solve(solver='ECOS')
        self.df.loc[0:len(self.df)-2, 'battery_operation'] = r.value
    ...
    def schedule_extra_storage(self, cap, max_rate):
        """Schedule battery added for capacity constraintes"""
        n = len(self.df)
        
        en = cp.Variable(n)  # energy
        r = cp.Variable(n-1)  # rate
        comb_demand = cp.Variable(n)  # net demand

        constraints = [en[0] == cap, r >= -max_rate, r <= max_rate, en >= 0, en <= cap] 
        for i in range(n-1):
            constraints.append(en[i+1] == en[i] + r[i])  # charging or discharging changes energy
            constraints.append(comb_demand[i] == self.df['unclipped_demand'].values[i] + r[i]) 
            constraints.append(comb_demand[i] <= self.df['capacity_clipval'].values[i])
            if self.df.loc[i, 'storage_demand'] > 0: # only discharge to meet capacity constraints
                constraints.append(r[i] == -self.df['storage_demand'].values[i])
            else:
                constraints.append(r[i] >= 0)
        constraints.append(comb_demand[n-1] == self.df['unclipped_demand'].values[n-1])

        prob = cp.Problem(cp.Minimize(cp.norm2(r)), constraints)  # spread out charging evenly where capacity

        print('Solving optimization.')
        self.res = prob.solve(solver='MOSEK')
#         print('Solving optimization. Note: using ECOS for now since MOSEK install not working.')
#         self.res = prob.solve(solver='ECOS')
        self.df.loc[0:n-2, 'battery_operation'] = r.value
        self.df.loc[n-1, 'battery_operation'] = 0
        self.df.loc[:, 'comb_demand_after_storage'] = comb_demand.value
...
class generatorData(object):
    def __init__(self, nerc, egrid_fname, eia923_fname, ferc714_fname='', ferc714IDs_fname='', cems_folder='', easiur_fname='', include_easiur_damages=True, year=2017, fuel_commodity_prices_excel_dir='', hist_downtime = True, coal_min_downtime = 12, cems_validation_run=False, testing_single_function=False, nan_price_cems_sorting=False, tz_aware=False):
        ...
        if year == 2019:
            ...
            egrid_year_str = str(21)
        elif year == 2020:
            egrid_year_str = str(20) # recently updated
        elif year == 2022:
            egrid_year_str = str(22)
        elif year >= 2014:
...
#                 temp_3.iloc[0] = temp_3.iloc[0].fillna(method='ffill') #for some reason the first row was not doing fillna(ffill)
    ...
    def addElecPriceToDemandData(self):
        ...
        year = self.year
        if int(self.year) >= 2022: # added 2022 case with new data format - Jan 17 2024
...
#         elif int(self.year) == 2021: #### 2021 case to be updated
#             print('***Implementing special case for ferc714 data in 2021, need to update for real***')
#             year = 2022
#             df_ids_bas = list(df_ids[df_ids.nerc == nerc_region].respondent_id.values)
#             #aggregate the price data by mean price per hour for any balancing authorities within the nerc region
#             df_bas = df[df.respondent_id.isin(df_ids_bas) & (df.report_yr==year)][['lambda_date', 'respondent_id', 'hour01', 'hour02', 'hour03', 'hour04', 'hour05', 'hour06', 'hour07', 'hour08', 'hour09', 'hour10', 'hour11', 'hour12', 'hour13', 'hour14', 'hour15', 'hour16', 'hour17', 'hour18', 'hour19', 'hour20', 'hour21', 'hour22', 'hour23', 'hour24']]
...
#             df_bas_temp['datetime'] = df_bas_temp['datetime'] + pandas.DateOffset(years=-1) # from 2022 back to 2021
...
#         df = df.append(df.iloc[-1])
        df = pandas.concat([df, pandas.DataFrame((df.loc[0]*0).to_dict(), index=[0])*2], axis=0)
...
#         df.loc[len(df)-1, 'demand'] = df.loc[len(df)-1, 'demand'] + 1000000 #creates a very large generator at the end of the merit order so that demand cannot be greater than supply
    ...
    def createMarginalPiecewise(self):
        ...
        test_shift.loc[:, 'demand'] = test_shift.demand + 0.1      
        ...
        df_marg_piecewise.loc[:, 'demand'] = df_marg_piecewise.demand - 0.1
    ...
    def createTotalInterpolationFunctions(self):
        ...
        """       
        test = self.df.copy()    
        last_zero = test.demand[test.demand == 0].index.values[-1] # zeros at the front for other reasons mess up new scipy interp1d, so start from last zero row
        test = test.loc[last_zero:, :]
        #cost
...
#         self.storage_df['demand'] = 0
        ...
        self.clip_vals_save = pandas.DataFrame({'datetime':demand_df['datetime'].values, 'clipval':numpy.zeros((len(demand_df), ))})
    ...
    def calcDispatchSlice(self, bstack, start_date=0, end_date=0):
        ...
        for col in self.df.columns: # make sure columns (except datetime and fuel type) are floats, not ints (initialized with 0 makes them ints, then pandas gives an error)
            if col == 'marg_gen_fuel_type':
                self.df[col] = self.df[col].astype(str)
            else:
                if col != 'datetime':
                    if self.df[col].dtype in ['int', 'int64']:
        ...
        if self.include_storage:
            ...
            self.clip_vals_save.loc[df_slice.index, 'clipval'] = clipval
#             print('Clipval: ', clipval)
            if df_slice.demand.max() > clipval:
                print('Clipping at: ', clipval)
...
#                 print('Maxval in storage_df:', self.storage_df.loc[inds, 'demand'].max())
    ...
    def createDfMdtCoal(self, demand_threshold, time_t):
        ...
        if self.bs.include_easiur:
            df_mdt_coal = self.bs.df_0[self.bs.df_0.orispl_unit.isin(coal_mdt_orispl_unit_list)].loc[:, ['orispl_unit', 'fuel', 'fuel_type', 'prime_mover', 'vom', 'min_out_multiplier', 'min_out', 'co2%i'%t, 'so2%i'%t, 'nox%i'%t, 'heat_rate%i'%t, 'mw%i'%t, 'fuel_price%i'%t, 'dmg%i'%t]].copy()
        else:
            df_mdt_coal = self.bs.df_0[self.bs.df_0.orispl_unit.isin(coal_mdt_orispl_unit_list)].loc[:, ['orispl_unit', 'fuel', 'fuel_type', 'prime_mover', 'vom', 'min_out_multiplier', 'min_out', 'co2%i'%t, 'so2%i'%t, 'nox%i'%t, 'heat_rate%i'%t, 'mw%i'%t, 'fuel_price%i'%t]].copy()
        ...
        df_mdt_coal_base.loc[:, ['orispl_unit', 'fuel', 'fuel_type', 'prime_mover', 'min_out_multiplier', 'min_out']] = ['coal_0', 'sub', 'coal', 'st', 0.0, 0.0]
        ...
        if self.bs.include_easiur:
            df_mdt_coal_base.loc[:, ['vom', 'co2%i'%t, 'so2%i'%t, 'nox%i'%t, 'heat_rate%i'%t, 'mw%i'%t, 'fuel_price%i'%t, 'dmg%i'%t]] = df_mdt_coal_base_temp.loc[:, ['vom', 'co2%i'%t, 'so2%i'%t, 'nox%i'%t, 'heat_rate%i'%t, 'mw%i'%t, 'fuel_price%i'%t, 'dmg%i'%t]]
        else:
            df_mdt_coal_base.loc[:, ['vom', 'co2%i'%t, 'so2%i'%t, 'nox%i'%t, 'heat_rate%i'%t, 'mw%i'%t, 'fuel_price%i'%t]] = df_mdt_coal_base_temp.loc[:, ['vom', 'co2%i'%t, 'so2%i'%t, 'nox%i'%t, 'heat_rate%i'%t, 'mw%i'%t, 'fuel_price%i'%t]]
        ...
        df_mdt_coal.loc[df_mdt_coal.fuel_type == 'coal','mw%i'%t] = df_mdt_coal[df_mdt_coal.fuel_type == 'coal'].loc[:, 'mw%i'%t].multiply((1-df_mdt_coal[df_mdt_coal.fuel_type == 'coal'].min_out_multiplier), axis='index')
    ...
    def calcDispatchAll(self, verbose=True, coal_downtime=True):
        ...
        #otherwise, run the dispatch in time slices, updating the bid stack each slice
            ...
            for t in self.time_array:
                ...
                if coal_downtime:
                    ...
                    coal_merit_order = self.bs.df[(self.bs.df.fuel_type == 'coal')].loc[:, ['orispl_unit', 'demand']]
